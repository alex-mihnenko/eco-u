...
if($argv[1]=='1'){
	// NEW GOODS /////////
	//PRODUCTS FROM BITRIX
	//mysql_query("truncate ms_cats");
	$CHECK_MS=true;
...
if($argv[1]=='2'){
	...
	//		     	if($vch['priceType']=='Старая цена') $price=$vch['value']/100;
				}		
				...
				//echo("select weight_class_id from oc_weight_class_description where title='".$EDIZM[$v['uom']['meta']['href']]['description']."'<BR>");
	...
	//				mysql_query("update  oc_product_description set name='".$v['name']."' where product_id='".$product_id."' and language_id='1'");
					mysql_query("update  oc_product_description set name='".$v['name']."' where product_id='".$product_id."' and language_id='1'");
                                                                                //meta_title='".addslashes($v['name'])."',  

					mysql_query("update  oc_product set date_modified='".date("Y-m-d H:i:s",time())."', 	manufacturer_id='".$CNTRS[$v['country']['meta']['href']]."', weight='".$v['weight']."',
					minimum='$minimum',weight_class_id='$uomid', price='".$price."', model='".$v['name']."' where product_id='$product_id'");                           ///model='".$v['name']."',sku='".$v['article']."',

/*
					$ressp=mysql_query("select  product_special_id from oc_product_special where product_id='$product_id'");
					list($chprsp)=mysql_fetch_row($ressp);
					if($chprsp){
						mysql_query("update oc_product_special set price='$price_old',customer_group_id='1',date_start='2017-03-01' where product_special_id='$chprsp'");
					}else{
						mysql_query("insert into oc_product_special set price='$price_old' , product_id='$product_id',customer_group_id='1',date_start='2017-03-01'");
					}
*/
				}else{
					if(isset($v['image']['meta']['href'])) {
					...
					mysql_query("insert into oc_product set minimum='$minimum', available='0', stock_status_id='5', 
					manufacturer_id='".$CNTRS[$v['country']['meta']['href']]."', weight='".$v['weight']."',
					weight_class_id='$uomid', price='".$price."', 
					date_added='".date("Y-m-d H:i:s",time())."', model='".$v['name']."',sku='".$v['externalCode']."',
					quantity='0', status='1' ,image= '".'catalog/'.$v['image']['filename']."'");
					$product_id=mysql_insert_id();	

					mysql_query("insert into oc_product_description set name='".addslashes($v['name'])."', meta_title='".addslashes($v['name'])."', product_id='".$product_id."', language_id='1',
...
/*
var_dump($NDEL_METKA);
$res=mysql_query("select ms_id from ms_products ");
while(list($CHID)=mysql_fetch_row($res)){

	if(!in_array($CHID,$NDEL_METKA)){
			mysql_query("update ms_products set  del='1' where product_id='$CHID'");
	}
}
*/
	...
	while($CHECK_MS){
		...
		foreach($json['rows'] as $k=>$v){
			...
			//		 	mysql_query("update oc_product_option_value set where product_option_value_id='$msvlid'");
					}
/////////
...
if($argv[1]=='3'){
	//mysql_query("update oc_product set quantity='0'  ");
