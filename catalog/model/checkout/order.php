...
class ModelCheckoutOrder extends Model {
        ...
        function __construct($registry) {
            ...
            $query = $this->db->query("SHOW INDEX FROM `" . DB_PREFIX . "order` WHERE key_name = 'order_status_id_date_added'");
            if(!$query->num_rows) {
                    $this->db->query("CREATE INDEX order_status_id_date_added ON `" . DB_PREFIX . "order` (`order_status_id`, `date_added`)");
            }
            
//            $this->db->query("UPDATE `" . DB_PREFIX . "order` "
//                    . "SET date_modified = NOW(), order_status_id = 1, `comment` = CONCAT('Брошенный заказ! ', `comment`) "
//                    . "WHERE order_status_id  = 0 AND date_added < DATE_SUB(NOW(), INTERVAL 1 HOUR)");
            $query_list = explode(";", "DROP TEMPORARY TABLE IF EXISTS tmp_order;
                DROP TEMPORARY TABLE IF EXISTS tmp_ip;
                CREATE TEMPORARY TABLE tmp_order 
                SELECT order_id, date_added, customer_id, ip FROM `" . DB_PREFIX . "order` 
                WHERE order_status_id  = 0 AND date_added < DATE_SUB(NOW(), INTERVAL 1 HOUR);
                CREATE TEMPORARY TABLE tmp_ip 
                SELECT o.ip FROM `" . DB_PREFIX . "order` o 
                LEFT JOIN tmp_order t ON t.ip = o.ip
                WHERE o.order_status_id > 0 AND o.date_added > t.date_added AND o.date_added < DATE_ADD(t.date_added, INTERVAL 1 HOUR);
                DELETE FROM tmp_order WHERE ip IN (SELECT ip FROM tmp_ip);
...
//            }
	...
	public function addOrder($data) {
		...
		if (isset($data['products'])) {
			foreach ($data['products'] as $product) {
				...
				if(!isset($product['variant'])) $product['variant'] = 1;
				$this->db->query("INSERT INTO " . DB_PREFIX . "order_product SET order_id = '" . (int)$order_id . "', product_id = '" . (int)$product['product_id'] . "', name = '" . $this->db->escape($product['name']) . "', model = '" . $this->db->escape($product['model']) . "', quantity = '" . (float)$product['quantity'] . "', amount = '" . (int)$product['amount'] . "', variant = '" . (float)$product['variant'] . "', price = '" . (float)$product['price'] . "', total = '" . (float)$product['total'] . "', tax = '" . (float)$product['tax'] . "', reward = '" . (int)$product['reward'] . "'");
	...
	public function addOrderHistory($order_id, $order_status_id, $comment = '', $notify = false, $override = false) {
		...
		if ($order_info) {
			...
			if (in_array($order_info['order_status_id'], array_merge($this->config->get('config_processing_status'), $this->config->get('config_complete_status'))) && !in_array($order_status_id, array_merge($this->config->get('config_processing_status'), $this->config->get('config_complete_status')))) {
				...
				foreach ($order_total_query->rows as $order_total) {
					if( $order_total['code'] != 'discount' && $order_total['code'] != 'discount_percentage' && $order_total['code'] != 'coupon_discount' ){
						$this->load->model('extension/total/' . $order_total['code']);
	...
	public function setDelivery($order_id, $customer_id, $data, $new_order_status = 1) {
        ...
        if( isset($this->session->data['shipping_address_2']) ) {
        	$order_data['shipping_address_2'] = json_encode($this->session->data['shipping_address_2'],JSON_UNESCAPED_UNICODE);
        }

        if( isset($this->session->data['shipping_custom_field']) && !empty($this->session->data['shipping_custom_field'])&& !is_null($this->session->data['shipping_custom_field']) ){
    ...
    public function getPersonalOrders($customer_id) {
        $sql = "SELECT *, (SELECT `name` FROM `".DB_PREFIX."order_status` st WHERE st.order_status_id = rd.order_status_id) AS `status_text`, (SELECT value FROM ".DB_PREFIX."order_total ot WHERE ot.order_id = rd.order_id AND ot.code = 'total' LIMIT 1) AS order_total FROM `".DB_PREFIX."order` rd WHERE `customer_id` = ".(int)$customer_id." AND order_status_id <> 0 ORDER BY order_id DESC LIMIT 30";
	...
	public function setPaymentCustomField($order_id, $payment_custom_field) {
		$this->db->query("UPDATE " . DB_PREFIX . "order SET payment_custom_field = '" . $payment_custom_field . "' WHERE order_id = '" . (int)$order_id . "'");
        return true;
	}

    public function addRoistatVisitId($order_id,$order_roistat_visit_id) {
        $this->db->query("INSERT INTO `" . DB_PREFIX . "order_roistat` SET order_id = '" . (int)$order_id . "', order_roistat_visit_id = '" . $this->db->escape($order_roistat_visit_id) . "'");
        return true;
    }
