...
class ModelCatalogProduct extends Model {
	public function updateViewed($product_id) {
		...
		$product_attribute_group_query = $this->db->query("SELECT ag.attribute_group_id, agd.name FROM " . DB_PREFIX . "product_attribute pa LEFT JOIN " . DB_PREFIX . "attribute a ON (pa.attribute_id = a.attribute_id) LEFT JOIN " . DB_PREFIX . "attribute_group ag ON (a.attribute_group_id = ag.attribute_group_id) LEFT JOIN " . DB_PREFIX . "attribute_group_description agd ON (ag.attribute_group_id = agd.attribute_group_id) WHERE pa.product_id IN (" . $str_product_id . ") AND agd.language_id = '" . (int)$this->config->get('config_language_id') . "' GROUP BY ag.attribute_group_id ORDER BY ag.sort_order, agd.name");
        ...
        public function getProductsByID($str_product_id) {
            $sql = "SELECT DISTINCT *, (SELECT attribute_id FROM " . DB_PREFIX . "product_attribute pa WHERE pa.product_id = p.product_id) AS attr_id, pd.customer_props3 AS customer_props3, pd.name AS name, p.image, m.name AS manufacturer, (SELECT name FROM " . DB_PREFIX . "attribute_description ad WHERE attr_id = pd.attribute_id) AS attribute_name, (SELECT sort_order FROM " . DB_PREFIX . "attribute ab WHERE attr_id = ab.attribute_id) AS attribute_sort, (SELECT price FROM " . DB_PREFIX . "product_discount pd2 WHERE pd2.product_id = p.product_id AND pd2.customer_group_id = '" . (int)$this->config->get('config_customer_group_id') . "' AND pd2.quantity = '1' AND ((pd2.date_start = '0000-00-00' OR pd2.date_start < NOW()) AND (pd2.date_end = '0000-00-00' OR pd2.date_end > NOW())) ORDER BY pd2.priority ASC, pd2.price ASC LIMIT 1) AS discount, (SELECT price FROM " . DB_PREFIX . "product_special ps WHERE ps.product_id = p.product_id AND ps.customer_group_id = '" . (int)$this->config->get('config_customer_group_id') . "' AND ((ps.date_start = '0000-00-00' OR ps.date_start < NOW()) AND (ps.date_end = '0000-00-00' OR ps.date_end > NOW())) ORDER BY ps.priority ASC, ps.price ASC LIMIT 1) AS special, (SELECT points FROM " . DB_PREFIX . "product_reward pr WHERE pr.product_id = p.product_id AND pr.customer_group_id = '" . (int)$this->config->get('config_customer_group_id') . "') AS reward, (SELECT ss.name FROM " . DB_PREFIX . "stock_status ss WHERE ss.stock_status_id = p.stock_status_id AND ss.language_id = '" . (int)$this->config->get('config_language_id') . "') AS stock_status, (SELECT wcd.unit FROM " . DB_PREFIX . "weight_class_description wcd WHERE p.weight_class_id = wcd.weight_class_id AND wcd.language_id = '" . (int)$this->config->get('config_language_id') . "') AS weight_class, (SELECT lcd.unit FROM " . DB_PREFIX . "length_class_description lcd WHERE p.length_class_id = lcd.length_class_id AND lcd.language_id = '" . (int)$this->config->get('config_language_id') . "') AS length_class, (SELECT AVG(rating) AS total FROM " . DB_PREFIX . "review r1 WHERE r1.product_id = p.product_id AND r1.status = '1' GROUP BY r1.product_id) AS rating, (SELECT COUNT(*) AS total FROM " . DB_PREFIX . "review r2 WHERE r2.product_id = p.product_id AND r2.status = '1' GROUP BY r2.product_id) AS reviews, p.sort_order FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) LEFT JOIN " . DB_PREFIX . "manufacturer m ON (p.manufacturer_id = m.manufacturer_id) WHERE p.product_id IN (" . $str_product_id . ") AND pd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND p.status = '1' AND p.date_available <= NOW() AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "'";
            var_dump($sql);
            die();
	...
	public function getProducts($data = array()) {
        ...
        public function searchProducts($search) {
            $arByName = $this->db->query("SELECT product_id FROM ".DB_PREFIX."product_description WHERE name LIKE '%".$this->db->escape($search)."%'");
            $arByShortDescription = $this->db->query("SELECT product_id FROM ".DB_PREFIX."product_description WHERE description_short LIKE '%".$this->db->escape($search)."%'");
            $arByTag = $this->db->query("SELECT product_id FROM ".DB_PREFIX."product_description WHERE tag LIKE '%".$this->db->escape($search)."%'");
            $items = array_merge($arByName->rows, $arByShortDescription->rows, $arByTag->rows);
            
            $result = Array();
            foreach($items as $item) {
                if(!isset($result[$item['product_id']])) {
                    $arProduct = $this->getProduct($item['product_id']);
                    if(($arProduct['quantity'] <= 0 && $arProduct['stock_status_id'] == 5) || $arProduct['status'] != 1) {
                        continue;
                    }
