...
class Order extends Base {
    ...
    public function info($order_id) {
        if (empty($order_id)) throw new InvalidDataException("Order ID cannot be empty for Order->info()");

        $postData = array(
        );

        $this->curl->setUrl($this->oc->getUrl('order/info&order_id='.$order_id));
        $this->curl->setData($postData);
        $this->curl->makeRequest();
        return $this->curl->getResponse();
    }

    public function products($order_id=0, $order_status_id = 0, $quantity = 0, $filter = array()) {
        $postData = array(
            'order_id' => $order_id,
            'order_status_id' => $order_status_id,
            'quantity' => $quantity,
        ...
        $this->curl->setUrl($this->oc->getUrl('order/products'));
        $this->curl->setData($postData);
        $this->curl->makeRequest();
        return $this->curl->getResponse();
    }
