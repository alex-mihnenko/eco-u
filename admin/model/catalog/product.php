...
class ModelCatalogProduct extends Model {
	public function addProduct($data) {
		// Discount
			if( isset($data['discount']) && intval($data['discount'])>0 ){
				$data['special_price'] = $data['price'] - ($data['price'] * (intval($data['discount'])/100));
			}
			else{
		...
		$this->db->query("INSERT INTO " . DB_PREFIX . "product SET model = '" . $this->db->escape($data['model']) . "', is_weighted = '" . $this->db->escape($data['is_weighted']) . "', composite_price = '" . $this->db->escape($data['composite_price']) . "', ultra_fresh = '" . $this->db->escape($data['ultra_fresh']) . "', yml = '" . $this->db->escape($data['yml']) . "', sku = '" . $this->db->escape($data['sku']) . "', weight_variants = '" . $this->db->escape($data['weight_variants']) . "', weight_package = '" . $data['weight_package'] . "', shelf_life = '" . $this->db->escape($data['shelf_life']) . "', available_in_time = '" . $this->db->escape($data['available_in_time']) . "', special_price = '" . $this->db->escape($data['special_price']) . "', discount = '" . $this->db->escape($data['discount']) . "', profitable_offer = '" . $this->db->escape($data['profitable_offer']) . "', available = '" . $this->db->escape($data['available']) . "', upc = '" . $this->db->escape($data['upc']) . "', ean = '" . $this->db->escape($data['ean']) . "', jan = '" . $this->db->escape($data['jan']) . "', isbn = '" . $this->db->escape($data['isbn']) . "', mpn = '" . $this->db->escape($data['mpn']) . "', location = '" . $this->db->escape($data['location']) . "', quantity = '" . (float)$data['quantity'] . "', minimum = '" . (int)$data['minimum'] . "', subtract = '" . (int)$data['subtract'] . "', stock_status_id = '" . (int)$data['stock_status_id'] . "', date_available = '" . $this->db->escape($data['date_available']) . "', manufacturer_id = '" . (int)$data['manufacturer_id'] . "', shipping = '" . (int)$data['shipping'] . "', price = '" . (float)$data['price'] . "', points = '" . (int)$data['points'] . "', weight = '" . (float)$data['weight'] . "', weight_class_id = '" . (int)$data['weight_class_id'] . "', length = '" . (float)$data['length'] . "', width = '" . (float)$data['width'] . "', height = '" . (float)$data['height'] . "', length_class_id = '" . (int)$data['length_class_id'] . "', status = '" . (int)$data['status'] . "', tax_class_id = '" . (int)$data['tax_class_id'] . "', sort_order = '" . (int)$data['sort_order'] . "', date_added = NOW(), date_new = NOW(), new = '" . (int)$data['new'] . "'");
		...
		foreach ($data['product_description'] as $language_id => $value) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "product_description SET product_id = '" . (int)$product_id . "', language_id = '" . (int)$language_id . "', name = '" . $this->db->escape($value['name']) . "', description = '" . $this->db->escape($value['description']) . "', customer_props3 = '" . $this->db->escape($value['customer_props3']) . "', description_short = '" . $this->db->escape($value['description_short']) . "', description_yml = '" . $this->db->escape($value['description_yml']) . "', tag = '" . $this->db->escape($value['tag']) . "', meta_title = '" . $this->db->escape($value['meta_title']) . "', meta_description = '" . $this->db->escape($value['meta_description']) . "', meta_keyword = '" . $this->db->escape($value['meta_keyword']) . "'");
		}
	...
	public function editProduct($product_id, $data) {
		...
		$this->db->query("
			...
			weight_package = '" .$data['weight_package'] . "', 
		...
		// Fix
			if( $data['new'] == 1 ) {
				$this->db->query("UPDATE " . DB_PREFIX . "product  SET date_new = NOW(), new = '1' WHERE product_id = '" . (int)$product_id . "'");
			}
			else { $this->db->query("UPDATE " . DB_PREFIX . "product  SET date_new = NOW(), new = '0' WHERE product_id = '" . (int)$product_id . "'"); }
			...
			if( isset($data['height']) ) 			$this->db->query("UPDATE " . DB_PREFIX . "product  SET height = '" . (float)$data['height'] . "' WHERE product_id = '" . (int)$product_id . "'");
			if( isset($data['length_class_id']) ) 	$this->db->query("UPDATE " . DB_PREFIX . "product  SET length_class_id = '" . (int)$data['length_class_id'] . "' WHERE product_id = '" . (int)$product_id . "'");
			if( isset($data['tax_class_id']) ) 		$this->db->query("UPDATE " . DB_PREFIX . "product  SET tax_class_id = '" . (int)$data['tax_class_id'] . "' WHERE product_id = '" . (int)$product_id . "'");
		// ---
	...
	public function getProducts($data = array()) {
		...
		if (isset($data['filter_quantity']) && !is_null($data['filter_quantity'])) {
			$sql .= " AND p.quantity = '" . (float)$data['filter_quantity'] . "'";
		}
	...
	public function getProductDescriptions($product_id) {
		...
		foreach ($query->rows as $result) {
			$product_description_data[$result['language_id']] = array(
                ...
                'description_yml'=> $result['description_yml'],
                'customer_props3'  => $result['customer_props3'],
